...
class VectorState {
 public:
  ...
  // VectorState(const VectorState<A, M> &state)
  //     : final_(state.Final()),
  //       niepsilons_(state.NumInputEpsilons()),
  //       noepsilons_(state.NumOutputEpsilons()),
  //       arcs_(state.arcs_) {}
  ...
  // For CLIF SharedPtr allocation.
  void *operator new(size_t size) {
    return ::operator new(size);
  }

  // For CLIF optional allocation.
  void *operator new(size_t size, VectorState<A, M> *state) {
    return ::operator new(size, state);
  }
